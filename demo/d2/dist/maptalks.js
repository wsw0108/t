...
var CanvasRenderer = function (_Class) {
    ...
    CanvasRenderer.prototype.needToRedraw = function needToRedraw() {
        ...
        if (map.isInteracting()) {
            return !(!map.getPitch() && map.isMoving() && !map.isZooming() && !this.layer.options['forceRenderOnMoving']);
        }
        return false;
    };
...
var Map = function (_Handlerable) {
    ...
    Map.prototype.setCenterAndZoom = function setCenterAndZoom(center, zoom) {
        if (!isNil(zoom) && this._zoomLevel !== zoom) {
            ...
            var a = this.options['zoomAnimation'];
...
var Painter = function (_Class) {
    ...
    Painter.prototype._getContainerPoints = function _getContainerPoints(points, dx, dy) {
        var map = this.getMap(),
            ...
            extent2D = containerExtent.expand(lineWidth / 2).convertTo(function (p) {
...
DrawTool.registerMode('rectangle', {
    ...
    'update': function update(coordinate, geometry) {
        ...
        var cnw = map.coordinateToPoint(firstCoord),
...
var MapBoxZoomHander = function (_Handler) {
    ...
    MapBoxZoomHander.prototype._boxZoom = function _boxZoom(param) {
        this.drawTool.remove();
        var extent = param.geometry.getExtent();
        var zoom = this.target.getFitZoom(extent);
...
Map.include({
    animateTo: function animateTo(view) {
        ...
        var player = this._animPlayer = Animation.animate(props, {
            ...
            if (player.playState === 'running') {
                ...
                if (!player._interupted) {
                    if (props['center']) {
                        _this._setPrjCenter(props['center'][1]);
                    }
                ...
                _this._endAnim(props, zoomOrigin);
        ...
        if (!this.isTransforming() && (props['pitch'] || props['bearing'])) {
            ...
            renderer.callInNextFrame(function () {
                preView = _this.getView();
                _this._startAnim(props, zoomOrigin);
            });
        } else {
    ...
    _endAnim: function _endAnim(props, zoomOrigin) {
        if (props['center']) {
            this.onMoveEnd();
        }
        if (!isNil(props['zoom'])) {
...
var TileLayer = function (_Layer) {
    ...
    TileLayer.prototype._getTiles = function _getTiles() {
        ...
        var mapOffset = map.getViewPoint();
...
TileLayer.mergeOptions(options$17);
...
DistanceTool.mergeOptions(options$18);
...
AreaTool.mergeOptions(options$19);
...
UIComponent.mergeOptions(options$20);
...
UIMarker.mergeOptions(options$21);
...
InfoWindow.mergeOptions(options$22);
...
var ToolTip = function (_UIComponent) {
    ...
    ToolTip.prototype.buildOn = function buildOn() {
        var dom = createEl('div');
        if (options$23.height) {
            dom.style.height = options$23.height + 'px';
        }
        if (options$23.width) {
            dom.style.width = options$23.width + 'px';
        }
        var cssName = this._cssName ? this._cssName : 'maptalks-tooltip';
        if (!this._cssName && options$23.height) {
            dom.style.lineHeight = options$23.height + 'px';
        }
...
ToolTip.mergeOptions(options$23);
...
Attribution.mergeOptions(options$24);
...
WMSTileLayer.mergeOptions(options$30);
...
CanvasTileLayer.mergeOptions(options$31);
...
var TileLayerDomRenderer = function (_Class) {
        ...
        if (zoomFraction !== 1) {
            ...
            if (map.isZooming() && this._zoomParam) {
                var origin = this._zoomParam['origin'],
                    pitch = map.getPitch(),
                    matOffset = [(origin.x - size['width'] / 2) * (1 - zoomFraction), (origin.y - size['height'] / 2) * (1 - zoomFraction) * (pitch ? Math.cos(pitch * Math.PI / 180) : 1), 0];
                translate(m, m, matOffset);
            }
    ...
    TileLayerDomRenderer.prototype._updateOpacity = function _updateOpacity() {
        ...
        for (var key in this._tiles) {
            ...
            var fade = Math.min(1, (now$$1 - tile.loaded) / 200);
...
typeof console !== 'undefined' && console.log('maptalks v0.27.0');
