...
var Extent = function () {
    ...
    Extent.prototype.round = function round() {
        return new this.constructor(Math.round(this['xmin']), Math.round(this['ymin']), Math.round(this['xmax']), Math.round(this['ymax']));
    };
...
function clipLine(points, bounds, useLastCode, round) {
    var parts = [];
    var k = 0,
        segment = void 0;
...
function withInEllipse(point, center, southeast, tolerance) {
    ...
    var a = Math.abs(southeast.x - center.x) * 2,
...
var Painter = function (_Class) {
    ...
    Painter.prototype.getPaintParams = function getPaintParams(dx, dy) {
        ...
        var containerPoints = this._getContainerPoints(points, dx, dy);
        if (!containerPoints) {
            return null;
        }
    ...
    Painter.prototype._getContainerPoints = function _getContainerPoints(points, dx, dy) {
        var map = this.getMap(),
            lineWidth = this.getSymbol()['lineWidth'] || 2,
            maxZoom = map.getMaxNativeZoom(),
            containerExtent = map.getContainerExtent(),
            extent2D = containerExtent.expand(lineWidth).convertTo(function (p) {
            return map._containerPointToPoint(p, maxZoom);
        }),
                ...
                if (this.geometry.getJSONType() === 'Polygon') {
                    ...
                    if (!Array.isArray(points[0])) {
                        ...
                        for (var _i = 0; _i < points.length; _i++) {
                            pushIn(clipPoints, clipLine(points[_i], extent2D, true));
                        }
                    }
                }
            }
            containerPoints = mapArrayRecursively(clipPoints, function (point) {
    ...
    Painter.prototype.getContainerExtent = function getContainerExtent() {
        ...
        var extent = this._extent2D.convertTo(function (c) {
            return map._pointToContainerPoint(c, zoom, _this2.height);
        });
        if (extent) {
...
var MapScrollWheelZoomHandler = function (_Handler) {
    ...
    MapScrollWheelZoomHandler.prototype._onWheelScroll = function _onWheelScroll(evt) {
        ...
        var nextZoom = map.getZoom() + levelValue;
        ...
        map.animateTo({
            ...
            if (_this2._requesting < 2 || Math.abs(nextZoom - _this2._startZoom) > 3) {
                map.animateTo({
                    'zoom': nextZoom,
...
var Path = function (_Geometry) {
    ...
    Path.prototype.animateShow = function animateShow() {
        ...
        var player = this._showPlayer = Animation.animate({
            ...
            _this2._drawAnimShowFrame(frame.styles.t, duration, length, animCoords);
            if (frame.state.playState === 'finished') {
...
var Rectangle = function (_Polygon) {
    ...
    Rectangle.prototype._getPrjShell = function _getPrjShell() {
        ...
        var fullExtent = projection.getFullExtent(),
            sx = fullExtent.sx,
            sy = fullExtent.sy;
    ...
    Rectangle.prototype._containsPoint = function _containsPoint(point, tolerance) {
        ...
        var extent = this._getPrjExtent()._expand(r);
    ...
    Rectangle.prototype._getSouthEast = function _getSouthEast(measurer) {
        ...
        var width = this.getWidth(),
            height = this.getHeight();
        var se = measurer.locate(this._coordinates, width, -height);
        return se;
    };
...
var OverlayLayer = function (_Layer) {
    ...
    OverlayLayer.prototype._sortGeometries = function _sortGeometries() {
        ...
        this._geoList.sort(function (a, b) {
            _this2._maxZIndex = Math.max(a.getZIndex(), b.getZIndex());
...
DrawTool.registerMode('boxZoom', {
    ...
    'update': function update(coordinate, geometry, param) {
        var map = geometry.getMap();
        var p1 = map.coordinateToContainerPoint(geometry._firstClick),
...
var MapBoxZoomHander = function (_Handler) {
    ...
    MapBoxZoomHander.prototype._boxZoom = function _boxZoom(param) {
        ...
        var extent = new Extent(center, map.locateByPoint(center, w, h));
...
TileLayer.mergeOptions(options$16);
...
Map.include({
    ...
    _calcMatrices: function _calcMatrices() {
        ...
        var domMat = copy(new Float64Array(16), m);
        ...
        m = create();
        scale(m, m, [this.width / 2, -this.height / 2, 1]);
        this.domCssMatrix = multiply(m, m, domMat);
    },
...
Map.include({
    ...
    getPreviousView: function getPreviousView() {
...
var UIMarker = function (_Handlerable) {
    ...
    UIMarker.prototype._getConnectPoints = function _getConnectPoints() {
        var map = this.getMap();
        var containerPoint = map.coordinateToContainerPoint(this.getCoordinates());
        var size = this.getSize(),
            width = size.width,
            height = size.height;
...
var Overview = function (_Control) {
    ...
    Overview.prototype.onAdd = function onAdd() {
        ...
        this.getMap().on('resize moving moveend zoomend pitch rotate', this._update, this).on('setbaselayer', this._updateBaseLayer, this);
        on(this.button, 'click', this._onButtonClick, this);
        this._updateButtonText();
    };

    Overview.prototype.onRemove = function onRemove() {
        this.getMap().off('resize moving moveend zoomend pitch rotate', this._update, this).off('setbaselayer', this._updateBaseLayer, this);
        if (this._overview) {
...
var GeometryEditor = function (_Eventable) {
    ...
    GeometryEditor.prototype._updateFromShadow = function _updateFromShadow() {
        if (!this._shadow) {
            return;
        }
    ...
    GeometryEditor.prototype._exeHistory = function _exeHistory(record) {
        ...
        if (Array.isArray(record[0])) {
            record[0].forEach(function (o) {
                ...
                _this8._geometry[m].apply(_this8._geometry, args);
            });
        } else {
...
var TileLayerDomRenderer = function (_Class) {
    ...
    TileLayerDomRenderer.prototype.drawOnInteracting = function drawOnInteracting() {
        ...
        var isAnim = map._animPlayer && map._animPlayer.duration >= this.layer.options['durationToAnimate'];
        if (isAnim) {
            ...
            if (map.isMoving()) {
                ...
                var gap = 2;
                var s = sign(this._endZoom - this._startZoom);
                var next = this._curAnimZoom + gap * s;
                ...
                if (this._endZoom < this._startZoom && next === tileZoom) {
                    ...
                    var nextZoom = this._curAnimZoom + gap * s;
                    if (nextZoom !== this._tileZoom && nextZoom >= this._endZoom) {
                        var curTilesCount = this.layer._getTiles().tiles.length;
                        var nextGrid = this.layer._getTiles(nextZoom);
    ...
    TileLayerDomRenderer.prototype._preloadTiles = function _preloadTiles(tiles) {
    ...
    TileLayerDomRenderer.prototype.needToRedraw = function needToRedraw() {
        if (this._redraw) {
            return true;
        }
        var map = this.getMap(),
    ...
    TileLayerDomRenderer.prototype.setToRedraw = function setToRedraw() {
        this._redraw = true;
    };
    ...
    TileLayerDomRenderer.prototype._renderTiles = function _renderTiles(escapeTileLoading) {
        this._redraw = false;
        if (!this._container) {
            this._createLayerContainer();
        }
        var tileGrid = this.layer._getTiles(escapeTileLoading ? this._tileZoom : undefined);
    ...
    TileLayerDomRenderer.prototype._getTileQueue = function _getTileQueue(tileGrid) {
        ...
        for (var i = tiles.length - 1; i >= 0; i--) {
            var cachedTile = this._tiles[tiles[i]['id']];
            if (cachedTile) {
    ...
    TileLayerDomRenderer.prototype._updateContainer = function _updateContainer() {
        var map = this.getMap(),
        ...
        if (!domMat) {
            ...
            if (centerOffset && !centerOffset.isZero()) {
                var offset = centerOffset.multi(fraction);
                style = Browser$1.any3d ? 'translate3d(' + offset.x + 'px, ' + offset.y + 'px, 0px) ' : 'translate(' + offset.x + 'px, ' + offset.y + 'px) ';
            }
            if (fraction !== 1) {
                var _matrix = [fraction, 0, 0, fraction, size['width'] / 2 * (1 - fraction), size['height'] / 2 * (1 - fraction)];
                style += 'matrix(' + _matrix.join() + ')';
            }
        ...
        if (Browser$1.chrome && Browser$1.chromeVersion.startsWith('60')) {
            var err = 'DOM TileLayer can\'t pitch or rotate due to a crash bug with chrome 60.';
            if (!this._reported) {
                this._reported = true;
        ...
        if (fraction !== 1) {
            ...
            if (map.isZooming() && this._zoomParam) {
                var origin = this._zoomParam['origin'],
                    pitch = map.getPitch(),
                    _offset = [(origin.x - size['width'] / 2) * (1 - fraction), (origin.y - size['height'] / 2) * (1 - fraction) * (pitch ? Math.cos(pitch * Math.PI / 180) : 1), 0];
                translate(m, m, _offset);
            }
            multiply(m, m, domMat);

            scale(m, m, [fraction, fraction, 1]);
            matrix = join(m);
        } else {
            matrix = join(domMat);
        }
        var mapOffset = map.getViewPoint().round();
        var tileOffset = void 0;
        if (map.isZooming() && !map.isMoving()) {
            var startFraction = map.getResolution(tileZoom) / map.getResolution(this._startZoom);
    ...
    TileLayerDomRenderer.prototype._updateOpacity = function _updateOpacity() {
        if (!this.getMap()) {
            return;
        }
    ...
    TileLayerDomRenderer.prototype._pruneLevels = function _pruneLevels() {
        var zoom = this._tileZoom;
    ...
    TileLayerDomRenderer.prototype._removeTileContainer = function _removeTileContainer(z) {
    ...
    TileLayerDomRenderer.prototype.onZoomStart = function onZoomStart(param) {
        var map = this.getMap();
        this._fadeAnimated = false;
        this._startZoom = map.getZoom();
    ...
    TileLayerDomRenderer.prototype.onZoomEnd = function onZoomEnd() {
        ...
        this._fadeAnimated = !Browser$1.mobile && this._startZoom !== this._endZoom;
    ...
    TileLayerDomRenderer.prototype._abortLoading = function _abortLoading(removeDOM) {
...
var MapCanvasRenderer = function (_MapRenderer) {
    ...
    MapCanvasRenderer.prototype._drawCanvasLayerOnInteracting = function _drawCanvasLayerOnInteracting(layer, t, timeLimit) {
        ...
        if (renderer.drawOnInteracting && (inTime || map.isZooming() && layer.options['forceRenderOnZooming'] || map.isMoving() && layer.options['forceRenderOnMoving'] || map.isRotating() && layer.options['forceRenderOnRotating'])) {
            renderer.prepareRender();
